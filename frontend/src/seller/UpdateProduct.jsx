...
import { useGetSpecificProductQuery, useUpdateProductMutation } from '../redux/api/products';
...
function UpdateProduct() {
    ...
    const { id } = useParams();
    const { data: productData, isLoading: productLoading } = useGetSpecificProductQuery(id);
    ...
    useEffect(() => {
        ...
        if (!productData?.product || !categories.length) return;

        const product = productData.product;

        const descriptionArray = Object.entries(product.description || {}).map(
            ([key, value]) => ({ key, value })
        );

        const categoryPath = findCategoryPath(categories, product.category_id);

        if (categoryPath) {

            const main = categoryPath[0]?._id || "";
            const sub = categoryPath[1]?._id || "";
            const last = categoryPath[2]?._id || "";

            setMainCategory(main);
            setSubCategory(sub);
            setLastCategory(last);

            const mainCat = categories.find(c => c._id === main);
            setSubCategories(mainCat?.subCategories || []);

            const subCat = mainCat?.subCategories?.find(c => c._id === sub);
            setLastCategories(subCat?.subCategories || []);
        }

        setFormData({
            name: product.name || "",
            description: descriptionArray.length ? descriptionArray : [{ key: "", value: "" }],
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await updateProduct({ id: id, data: finalProductData });
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" className="w-full sm:p-4 p-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-orange-500 outline-none" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-gray-600 mb-2">Brand Name</label>
                            <input type="text" className="w-full sm:p-4 p-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-orange-500 outline-none" value={formData.brand} onChange={(e) => setFormData({ ...formData, brand: e.target.value })} required />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" className="w-full sm:p-4 p-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-orange-500 outline-none" value={formData.mrp_price} onChange={(e) => setFormData({ ...formData, mrp_price: e.target.value })} required />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-gray-600 mb-2">Price (₹)</label>
                            <input type="number" className="w-full sm:p-4 p-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-orange-500 outline-none" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })} required />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" className="w-full sm:p-4 p-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-orange-500 outline-none" value={formData.stock} onChange={(e) => setFormData({ ...formData, stock: e.target.value })} required />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-gray-600 mb-2">SKU</label>
                            <input type="text" className="w-full sm:p-4 p-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-orange-500 outline-none" value={formData.sku} onChange={(e) => setFormData({ ...formData, sku: e.target.value })} required />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-gray-600 mb-2">Tax (%)</label>
                            <input type="number" className="w-full sm:p-4 p-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-orange-500 outline-none" value={formData.tax} onChange={(e) => setFormData({ ...formData, tax: e.target.value })} required />
                        </div>
                    </div>
